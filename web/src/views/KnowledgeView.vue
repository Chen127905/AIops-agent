...
import { ingestDocument, publishDocumentVersion, searchKnowledge, type EvidenceChunk } from '../api/knowledge'
import ConsoleLayout from '../components/ConsoleLayout.vue'
import ModalDialog from '../components/ModalDialog.vue'

const query = ref('Redis 连接池超时如何排查')
const results = ref<EvidenceChunk[]>([])
const error = ref(''); const notice = ref(''); const searched = ref(false); const ingesting = ref(false)
...
async function ingest(): Promise<void> {
  ...
  try {
    const id = dialog.value === 'version' ? await publishDocumentVersion(Number(document.documentId), command) : await ingestDocument(command)
...
<template><ConsoleLayout>
  <header class="console-header"><div><p class="section-label">RAG 知识中心</p><h2>知识库</h2><p class="header-copy">管理已发布运维知识，并验证 Agent 实际能够召回的证据片段。</p></div><div class="header-actions"><button class="secondary-action" @click="open('version')">发布新版本</button><button class="primary-button" @click="open('create')">＋ 新增文档</button></div></header>
  ...
  <ModalDialog v-if="dialog" :title="dialog === 'create' ? '新增知识文档' : '发布文档新版本'" eyebrow="知识入库" wide @close="dialog = null">
    <form class="form-stack" data-test="ingest-form" @submit.prevent="ingest">
      <label v-if="dialog === 'version'"><span>原文档 ID</span><input v-model="document.documentId" type="number" min="1" placeholder="例如 12" required><small>新版本会替换该文档当前可检索的已发布版本</small></label>
