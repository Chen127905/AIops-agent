...
import { getAgentTask } from '../api/tickets'
...
async function decide() {
  ...
  try { decision.value === 'approve' ? await approve(selected.value.id, comment.value) : await reject(selected.value.id, comment.value); selected.value = null; const task = await getAgentTask(taskId); await router.push({ path: `/tickets/${task.ticketId}`, query: { task: task.id } }) }
  catch { error.value = '审批已被其他人处理或已经过期' } finally { deciding.value = false }
}
async function viewTicket(item: Approval): Promise<void> { const task = await getAgentTask(item.taskId); await router.push({ path: `/tickets/${task.ticketId}`, query: { task: task.id } }) }
onMounted(() => void load())
</script>
