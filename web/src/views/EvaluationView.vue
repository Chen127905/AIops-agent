...
<template><ConsoleLayout>
  ...
  <section v-if="run" class="run-summary"><div class="section-heading"><div><p class="section-label">评测结果</p><h3>{{ displayLabel(run.mode) }} · {{ run.model }}</h3></div><div class="run-meta"><span class="status-pill" :data-status="run.status">{{ displayLabel(run.status) }}</span><span>{{ formatDate(run.finishedAt ?? run.startedAt) }}</span></div></div>
    <div class="metric-grid"><article><span>用例通过率</span><strong>{{ (run.metrics.passRate * 100).toFixed(1) }}%</strong><small>{{ run.metrics.passedCases }} / {{ run.metrics.totalCases }} 通过</small></article><article><span>工具 Precision / Recall</span><strong>{{ run.metrics.toolPrecision.toFixed(2) }} / {{ run.metrics.toolRecall.toFixed(2) }}</strong><small>工具选择质量</small></article><article><span>引用准确率</span><strong>{{ (run.metrics.citationAccuracy * 100).toFixed(1) }}%</strong><small>证据引用正确性</small></article><article><span>审批拦截率</span><strong>{{ (run.metrics.approvalInterceptionRate * 100).toFixed(1) }}%</strong><small>高风险动作拦截</small></article><article><span>敏感信息泄露</span><strong>{{ run.metrics.leakageCount }}</strong><small>目标值为 0</small></article><article><span>P95 延迟</span><strong>{{ run.metrics.p95LatencyMs }}</strong><small>ms</small></article></div>
  ...
  <ModalDialog v-if="showRun" title="创建评测运行" eyebrow="评测配置" wide @close="showRun = false"><form class="form-stack" @submit.prevent="requestExecution"><div class="mode-selector"><label :class="{ selected: form.mode === 'MOCK' }"><input v-model="form.mode" type="radio" value="MOCK" @change="confirmingLive = false"><span><strong>基线评测</strong><small>确定性执行，不调用模型 API，适合快速回归</small></span></label><label :class="{ selected: form.mode === 'LIVE' }"><input v-model="form.mode" type="radio" value="LIVE"><span><strong>真实模型评测</strong><small>调用模型 API，验证真实效果并产生费用</small></span></label></div><div v-if="form.mode === 'LIVE'" class="form-columns"><label><span>模型提供商</span><select v-model="form.provider"><option value="QWEN">阿里云百炼 · Qwen</option><option value="DEEPSEEK">DeepSeek</option></select></label><label><span>模型名称（可选）</span><input v-model="form.model" maxlength="128" placeholder="留空使用默认模型"></label></div><div class="form-columns"><label><span>Prompt 版本</span><input v-model="form.promptVersion" maxlength="64" required></label><label><span>知识版本</span><input v-model="form.knowledgeVersion" maxlength="64" required></label></div><div class="case-selector-heading"><div><strong>选择用例</strong><small>不选择时由后端运行全部用例</small></div><button class="text-button" type="button" @click="selectAll">{{ form.caseIds.length === cases.length ? '取消全选' : '全选' }}</button></div><div class="checkbox-grid"><label v-for="item in cases" :key="item.id"><input v-model="form.caseIds" type="checkbox" :value="item.id"><span><strong>{{ caseTitle(item) }}</strong><small>{{ displayLabel(item.group) }} · {{ item.id }}</small></span></label></div><div v-if="confirmingLive" class="cost-warning"><strong>确认调用真实模型 API</strong><p>本次运行会调用 {{ form.provider }} 并产生 Token 费用。请确认模型 Key、预算和所选用例后再次提交。</p></div><div class="modal-actions"><button class="text-button" type="button" @click="showRun = false">取消</button><button class="primary-button" :disabled="running">{{ running ? '评测运行中…' : (confirmingLive ? '确认并开始真实评测' : '开始评测') }}</button></div></form></ModalDialog>
