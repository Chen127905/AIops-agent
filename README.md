...
审批是一条独立于模型推理的确定性业务链路。Agent 提出 `restartService` 或 `changeConfig` 后，任务进入 `WAITING_APPROVAL`，平台创建带有效期的审批记录。当前由同租户内已认证用户批准或拒绝，同一审批只允许一个决策成功；生产环境建议增加独立 `APPROVER` 角色。
...
7. 如果目标系统已配置获批变更端点且模型建议的是白名单动作，任务进入审批；租户内已认证用户批准后平台执行动作，再次检查健康状态并决定是否关闭工单。
...
- [项目全景、实现原理与 Agent 面试指南](docs/project-design-and-interview-guide.md)
